...
public class MemoryStoneManager implements Listener, StructureListener {
    ...
    public void structureLoaded(Structure structure, LinkedHashMap<String, Object> node) {
        MemoryStone stone = new MemoryStone();
        stone.setStructure(structure);
...
//        }
        ...
        if (node.containsKey("signx")) {
            try {
                int x = (Integer)node.get("signx");
                int y = (Integer)node.get("signy");
                int z = (Integer)node.get("signz");
